	...
	*/
	var makeBoard = function (numRows, numColumns) {
		...
		for (var i = 0; i < cell.length; i++) {
			squares[i] = cell[i];	//Adds the cells to the squares array
			squares[i].indicator = indicator;	
			console.log(cell[i].indicator);
			indicator++;
			}
	...
	var setSquare = function (square) {
		...
		squares[square].isSelected = false;//ensures that a selected square has been drawn
	};
